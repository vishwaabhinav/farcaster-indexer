...
import { Reaction, FlattenedReaction, MerkleResponse } from '../types/index'
 ...
 */
export async function indexAllLikes(casts: string[], limit?: number) {
...
async function formatAndSave(allLikes: Array<any>) {
  ...
  const chunks = breakIntoChunks(formattedLikes, 1000)
...
async function getAllLikes(casts: string[], limit?: number): Promise<number> {
  const chunks = breakIntoChunks(casts, 300);
  let count = 0;
  ...
  for (const castChunk of chunks) {
    ...
    await formatAndSave(likes);
    count += likes.length;

    await new Promise(r => setTimeout(r, 15000));

    console.log(`[${count * 100 / casts.length}%] ${count} of ${casts.length} processed`);
    // console.log(likes);
  }
...
function buildLikeEndpoint(cast: string, cursor?: string): string {
  return `https://api.farcaster.xyz/v2/cast-likes?castHash=${cast}&limit=100${cursor ? `&cursor=${cursor}` : ''
    }`
}
